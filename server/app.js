...
// Shared by the 09:30/11:30 admin "send now" button AND the external-cron-triggered
// endpoints in api/cron/*.js — kept here so both call the exact same logic.
...
// Shared by the local node-cron 06:45 job AND the Vercel cron endpoint —
// fires till tickets for On-Site Café orders placed in advance for today.
async function runCafeMorningJob() {
  const menu = await store.readMenu();
  const site = getSite(menu, 'onsite');
  if (!site || site.ticketMode !== 'immediate' || !square.isConfigured()) {
    return { ran: false, reason: 'Café site not found, not immediate-ticket mode, or Square not configured' };
  }
  const todayKeyValue = todayKey(site.settings.timezone);
  ...
  for (const order of orders) {
    ...
    try {
      ...
      console.error(`Advance café ticket failed for order ${order.id}:`, err.message);
    }
  }
...
async function handleBundleOrder(req, res, site) {
  ...
  if (deliveryFeatureOn && fulfilment === 'delivery') {
    if (!deliveryAddress || !deliveryAddress.trim()) {
      return res.status(400).json({ error: 'Please enter where you\'d like your order delivered.' });
    }
...
async function handleCartOrder(req, res, site) {
  ...
  if (square.isConfigured()) {
    try {
      const payment = await square.createPayment({
        ...
        idempotencyKey: order.id,
        note: `On-Site Café — ${order.name} — ${lines.map(l => `${l.qty}x ${l.name}`).join(', ')}${isForToday ? '' : ' — for ' + resolvedForDate}`.slice(0, 500)
      });
      order.status = 'paid';
      order.squarePaymentId = payment.id;
    } catch (err) {
  ...
  if (isForToday && site.ticketMode === 'immediate' && square.isConfigured()) {
    try {
      ...
      ticket = { attempted: true, sent: false, error: err.message };
      console.error('On-Site Café ticket failed:', err.message);
    }
  } else if (!isForToday) {
