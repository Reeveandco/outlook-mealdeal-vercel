// Sends the order-summary emails via Gmail SMTP (nodemailer), using theoutlookatfoxs@gmail.com.
//
// Set these as environment variables on whatever server this runs on:
//   GMAIL_USER          - the Gmail address the emails are sent FROM (theoutlookatfoxs@gmail.com)
//   GMAIL_APP_PASSWORD  - a 16-character Google App Password for that account (NOT the normal
//                         Gmail password). Requires 2-Step Verification to be turned on for the
//                         account first, then generate one at myaccount.google.com/apppasswords.
//
...
// rest of the system can still be tested and demoed without Gmail set up yet.
...
async function sendRawEmail({ to, cc, subject, text }) {
    if (!isConfigured()) {
          console.log('=== [DRY RUN - no Gmail credentials configured] ===');
  ...
  await transporter.sendMail({
        from: `"The Outlook at Fox's" <${process.env.GMAIL_USER}>`,
        to,
